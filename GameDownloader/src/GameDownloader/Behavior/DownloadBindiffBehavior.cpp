  ...
  namespace GameDownloader {
    namespace Behavior {
      ...
      void DownloadBindiffBehavior::setTorrentWrapper(P1::Libtorrent::Wrapper *wrapper)
      {
        ...
        // ѕросто реемитим ошибку безликую, при желании можно что-нить поконкретнее пробросить
      ...
      void DownloadBindiffBehavior::syncStartTorrent(P1::GameDownloader::ServiceState *state)
      {
        ...
        config.setIsReloadRequired(false);  // UNDONE - хз может пон€ть что файл изменилс€ и в этом случаи перекачать
      ...
      void DownloadBindiffBehavior::torrentProgress(P1::Libtorrent::EventArgs::ProgressEventArgs arg)
      {
        ...
        // UNDONE тут бы надо помен€ть сообщение, на провер€ю обновление
        if (isRehashing)
